...
interface ProductPublishingSidebarProps {
  isActive: boolean;
...
export default function ProductPublishingSidebar({
  isActive,
  onToggleActive,
  product,
  isEditMode,
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50">
            <div>
              ...
              <p className="text-xs text-slate-500">
                {isEditMode ? 'Currently live on store' : 'New product'}
              </p>
            </div>
            <button
              type="button"
          ...
          {isEditMode && (
            <button
              type="button"
              onClick={onDelete}
              className="w-full mt-4 flex items-center justify-center gap-2 text-red-500 hover:text-red-600 text-xs font-bold transition-colors"
            >
